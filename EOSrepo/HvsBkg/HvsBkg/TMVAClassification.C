...
int TMVAClassification(TString myMethodList = "") {
  ...
  TFile *input(0);
  TString fname = "./HvsZ_Histograms.root";
  ...
  TTree *zbkgTree = (TTree *)input->Get("Ztree_" + decay0 + "+" + decay1);

  Int_t signalMCEntries =
      static_cast<TH1F *>(input->Get("H_MC_histograms")->FindObject("h_mcMInv"))
          ->GetEntries();
  Int_t zMCEntries =
      static_cast<TH1F *>(input->Get("Z_MC_histograms")->FindObject("h_mcMInv"))
          ->GetEntries();
  ...
  std::cout << "Number of expected signal events: " << n_signalExp << '\n';
  std::cout << "Number of expected bkg events: " << n_zbkgExp << '\n';
