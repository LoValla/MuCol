...
void Histo_filler(const TString &decay = "all",
  ...
  for (int ientry = 0; ientry < nEntries; ++ientry) {
    ...
    for (unsigned int i = 0; i < ntau_mc; ++i) {
      switch (mcDecayMode[i]) {
      case 0:
      case 1:
      case 2:
      case 3:
        mymcDecayMode[i] = 0;
        if (decay != "all" && decay != "1pi")
          continue;
        break;
      case 4:
      case 5:
        mymcDecayMode[i] = 1;
        if (decay != "all" && decay != "3pi")
          continue;
        break;
      case 6:
        mymcDecayMode[i] = 2;
        if (decay != "all" && decay != "e")
          continue;
        break;
      case 7:
        mymcDecayMode[i] = 3;
        if (decay != "all" && decay != "mu")
          continue;
        break;
      default:
        mymcDecayMode[i] = 4;
        if (decay != "all" && decay != "other")
          continue;
        break;
      }
  ...
  for (int ientry = 0; ientry < nEntries; ++ientry) {
    ...
    if (TMath::Sqrt(pow((recEta[0] - mcEta[0]), 2) + pow(deltaPhi, 2)) >= 0.1)
      continue;
    ...
    if (recDecayMode <= 3) {
      myrecDecayMode = 0;
      if (decay != "all" && decay != "1pi" &&
          decay != "1pi" + std::to_string(recDecayMode - 1) + "N") {
        continue;
      }
    } else if (recDecayMode == 4) {
      myrecDecayMode = 1;
      if (decay != "all" && decay != "3pi") {
        continue;
      }
    } else if (recDecayMode == 5) {
      myrecDecayMode = 2;
      if (decay != "all" && decay != "e") {
        continue;
      }
    } else if (recDecayMode == 6) {
      myrecDecayMode = 3;
      if (decay != "all" && decay != "mu") {
        continue;
      }
    ...
    for (unsigned int i = 0; i < ntau_mc; ++i) {
      if (mcDecayMode[i] == 0 || mcDecayMode[i] == 1 || mcDecayMode[i] == 2 ||
          mcDecayMode[i] == 3)
        mymcDecayMode[i] = 0;
      else if (mcDecayMode[i] == 4 || mcDecayMode[i] == 5)
        mymcDecayMode[i] = 1;
      else if (mcDecayMode[i] == 6)
        mymcDecayMode[i] = 2;
      else if (mcDecayMode[i] == 7)
        mymcDecayMode[i] = 3;

      mcEta[i] = std::fabs(mcEta[i]);

      if (decay == "all" || mymcDecayMode[i] == myrecDecayMode) {
        // fix eta, draw histos for E, p
        if (mcEta[i] >= 0. && mcEta[i] <= 0.5) {
  ...
  if (TEfficiency::CheckConsistency(*h_EvsEta0_mcHits, *h_EvsEta0_mcAll)) {
    ...
    c_EvsEta0_eff->SetBottomMargin(0.12);
  ...
  if (TEfficiency::CheckConsistency(*h_EtavsE1_mcHits, *h_EtavsE1_mcAll)) {
    ...
    eff_EtavsE1->SetTitle(
        ";|#eta_{MC}^{vis}(#tau)|; #tau reconstruction efficiency");
    graph->GetXaxis()->SetTitleOffset(1.2);
    graph->GetXaxis()->SetTitleSize(0.045);
    graph->GetXaxis()->SetLabelSize(0.045);
    graph->GetXaxis()->SetLabelOffset(0.012);
    ...
    TPaveText *pt1 = new TPaveText(1.42, 0.6, 2.7, 0.97);
    pt1->SetFillColor(0);
    pt1->SetMargin(0);
    pt1->SetFillStyle(0);
    pt1->SetBorderSize(0);
  ...
  auto c_ECorr_Eta0 = new TCanvas("c_ECorr_Eta0", "c_ECorr_Eta0", 800, 800);
  gStyle->SetOptStat(0);
  h_ECorr_Eta0->GetXaxis()->SetTitleOffset(1.2);
  h_ECorr_Eta0->SetMinimum(0.95);
  h_ECorr_Eta0->Draw("E1");
  // gPad->SetLogz(1);
  gPad->SetLogx();
  c_ECorr_Eta0->SetLeftMargin(0.15);
  c_ECorr_Eta0->SaveAs("plots_reco_" + decay + "/c_ECorr_Eta0.png");
  c_ECorr_Eta0->Close();
  ...
  // file
  TDirectory *dir = outputFile->GetDirectory("tauguns_notrackfiltering");
  dir->cd();
...
void TauRecoAnalysis(const TString filename = "output_EvalTauFinder.root") {
  // Histo_filler("all", filename);
  // Histo_filler("e", filename);
  // Histo_filler("mu", filename);
  Histo_filler("1pi", filename);
