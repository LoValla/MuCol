...
void Histo_filler(const TString &decay = "N",
  ...
  if (TEfficiency::CheckConsistency(*h_EvsEta0_mcHits, *h_EvsEta0_mcAll)) {
    ...
    pt3->SetTextAlign(31);
    // pt3->SetTextFont(50);
    pt3->AddText("#bf{h^{#pm} exclusive decays}");
  ...
  if (TEfficiency::CheckConsistency(*h_EtavsE1_mcHits, *h_EtavsE1_mcAll)) {
    ...
    // pt1->AddText("  ");
    pt1->AddText("#bf{h^{#pm} exclusive decays}");
  ...
  h_ECorr_Eta0->SetMinimum(0.97);
  h_ECorr_Eta0->SetMaximum(1.03);
  ...
  TPaveText *pt4 = new TPaveText(80, 1.008, 700, 1.028);
  pt4->SetFillColor(0);
  pt4->SetMargin(0);
  pt4->SetFillStyle(0);
  pt4->SetBorderSize(0);
  ...
  pt4->AddText("  ");
  pt4->AddText("#bf{h^{#pm} exclusive decays}");
  ...
  // file
  TDirectory *dir = outputFile->GetDirectory("tauguns_notrackfiltering_0N");
  dir->cd();
...
void TauRecoAnalysis_0N(const TString filename = "output_EvalTauFinder.root") {
