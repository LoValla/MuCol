...
TH1F *h_deltaR_tsmc, *h_deltaR_tscl, *h_deltaR_tscl_pi, *h_deltaR_tscl_neu,
    *h_deltaR_tscl_others, *h_mc_phi, *h_mc_eta, *h_mc_theta, *h_mc_pt;
...
void lctuplePiAnalysis(const TString filename = "output_lctuple.root") {
  ...
  // Histogram initialization
  {
    ...
    h_recopdg =
        new TH1I("h_recopdg", "Pdg of reconstructed particles; ; events", 11,
                 -0.5, 10.5);
    h_mc_phi = new TH1F("h_mc_phi", "Phi of mc particles; ; events", 40, - M_PI, M_PI);
    h_mc_eta = new TH1F("h_mc_eta", "Eta of mc particles; ; events", 40, -2.5,
                        2.5);
    h_mc_theta = new TH1F("h_mc_theta", "Theta of mc particles; ; events", 40, 0.,
                        180.);
    h_mc_pt = new TH1F("h_mc_pt", "p_{T} of mc particles; ; events", 50, 0.,
                        500.);
  }
  ...
  for (int ientry = 0; ientry < nEntries; ++ientry) {
    ...
    for (int i = 0; i < n_mcp; ++i) {
      ...
      h_mc_phi->Fill(mcPhi[n_mcp_stable]);
  ...
  // draw the histograms
  {
    ...
    TCanvas *c_mc_phi = new TCanvas("c_mc_phi", "c_mc_phi", 800, 800);
    gStyle->SetOptStat(111111);
    h_mc_phi->Draw();
    c_mc_phi->SaveAs("./plots/c_mc_phi.png");
    c_mc_phi->Close();
