...
void TauDMAnalysis(
  ...
  for (int col = 1; col != nRecDecayModes + 1; col++) {
    int tot_col_events = h_DecayMode_RecvsMc->Integral(col, col, 1, 9);
    std::cout << "col" << col << ": " << tot_col_events << '\n';
    if (tot_col_events == 0)
      continue;

    for (int row = 1; row != 9 + 1; row++) {
      int bin_events = h_DecayMode_RecvsMc->GetBinContent(col, row);

      std::cout << "float(bin_events) / float(tot_col_events) = "
                << float(bin_events) / float(tot_col_events) << '\n';
      h_DecayMode_RecvsMc->SetBinContent(
          col, row, float(bin_events) / float(tot_col_events) * 1000);
    }
  }

  {
    const char *rec_labels[nRecDecayModes] = {
        "#pi^{#pm}",           "#pi^{#pm} + N", "#pi^{#pm} + 2N",
        "#pi^{#pm} + #geq3N", "3#pi^{#pm}",     "e^{#pm}",
        "#mu^{#pm}",           "Others"};
    const char *mc_labels[9] = {"#pi^{#pm}",
                                "#pi^{#pm} + #pi^{0}",
                                "#pi^{#pm} + 2#pi^{0}",
                                "#pi^{#pm} + 3#pi^{0}",
                                "3#pi^{#pm}",
    ...
    auto c_DecayMode_RecvsMc =
        new TCanvas("c_DecayMode_RecvsMc", "c_DecayMode_RecvsMc", 1000, 800);
    gStyle->SetOptStat(0);
    h_DecayMode_RecvsMc->Draw("COL TEXT");
    // h_DecayMode_RecvsMc->GetXaxis()->SetBit(TAxis::kLabelsHori);
    for (unsigned int i = 1; i <= 9; i++)
      h_DecayMode_RecvsMc->GetYaxis()->SetBinLabel(i, mc_labels[i - 1]);
    for (unsigned int j = 1; j <= nRecDecayModes; j++)
      h_DecayMode_RecvsMc->GetXaxis()->SetBinLabel(j, rec_labels[j - 1]);
    h_DecayMode_RecvsMc->GetXaxis()->SetLabelSize(0.045);
    h_DecayMode_RecvsMc->GetYaxis()->SetLabelSize(0.045);
    c_DecayMode_RecvsMc->SaveAs("plots/c_DecayMode_RecvsMc.png");
    c_DecayMode_RecvsMc->Close();
  }
