...
TauFinder::TauFinder() : Processor("TauFinder") {
  ...
  _description =
      "TauFinder writes tau candidates as ReconstructedParticles into "
  ...
  registerProcessorParameter("isolationConeAngle",
                             ...
                             "jet in rad (relativ to cone angle)",
...
void TauFinder::processRunHeader(LCRunHeader *) { _nRun++; }
...
void TauFinder::processEvent(LCEvent *evt) {
  ...
  // TauFinder in action (only if there's at least 1 charged particle)
  ...
  for (unsigned int p = 0; p < tauvec.size(); p++) {
    ...
    if (mass_inv > _minv || mass_inv < -0.001 || chargedtracks > 4 ||
        chargedtracks == 0) {
      if (mass_inv > _minv)
        _fail_minv++;
      if (mass_inv < -0.001)
...
bool TauFinder::FindTau(
  ...
  double OpAngleMax = 0;
  // find a good tauseed (check impact minimum pt)
  ...
  //if no good tau candidate, return finding_done = true
  if (!tauseed) {
    if (_nEvt < coutUpToEv || _nEvt == coutEv)
...
void TauFinder::end() {
  ...
  streamlog_out(MESSAGE) << "TauFinder::end()  " << name() << " processed "
